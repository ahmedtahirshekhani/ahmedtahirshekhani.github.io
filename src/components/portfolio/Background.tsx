...
export default function Background() {
  ...
  const [particles, setParticles] = useState<{ left: number; top: number }[]>([]); // To store the random positions for particles
  ...
  useEffect(() => {
    // Generate random particle positions only on the client side
    const generatedParticles = Array.from({ length: isMobile ? 5 : 12 }, () => ({
